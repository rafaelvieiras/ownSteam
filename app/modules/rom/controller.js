angular.module('RomApp').controller('RomShowCtrl', RomShowCtrl);

RomShowCtrl.$inject = ['$scope', '$routeParams', '$filter', '$http', 'electron', '$mdDialog', '$mdMedia', 'RomService', 'EmulatorService'];

function RomShowCtrl($scope, $routeParams, $filter, $http, electron, $mdDialog, $mdMedia, RomService, EmulatorService) {
  var vm = this;
  ...
  function start(){
    ...
    /**
     * Call Rom Service, and use action get to put rom data in vm
     * @return
     */
    RomService.get({ _id: $routeParams.id }).then(function(data){
      console.log(data);
      vm.rom = data;

      EmulatorService.get({ _id: data.emulator }).then(function(data){
        vm.emulator = data;
  ...
  function scrapRom(){
    ...
    vm.rom.release = obj.original_release_date;
    ...
    //   var newFile = __dirname + '/data/images/emulator/' + vm.emulator.slug +'/'+ $filter('escape')(obj.name) + img.split('.').pop();
    ...
    // var newFile = __dirname + '/data/images/emulator/' + vm.emulator.slug + electron.path.extname(path);
    ...
    // if(exist)
    //   vm.emulator.mainImage = 'data/images/emulator/' + vm.emulator.slug + electron.path.extname(path);
  ...
  function play(){
    var path = vm.emulator.path + ' "' + vm.emulator.command.replace("&romPath", vm.rom.path) + '"';
    // var path = path.replace("&romPath", vm.rom.path);
    var exec = require('child_process').exec;
    exec(path, function callback(error, stdout, stderr){
      console.log(error);
      console.log(stdout);
      console.log(stderr);
    });
  ...
  function configure(){
    var useFullScreen = ($mdMedia('sm') || $mdMedia('xs'))  && vm.customFullscreen;
    $mdDialog.show({
      controller: 'RomConfigureCtrl',
      controllerAs: 'ctrl',
      templateUrl: 'modules/rom/views/configure.html',
      parent: angular.element(document.body),
      clickOutsideToClose: true,
      fullscreen: useFullScreen,
      locals: {
...
var RomScraperCtrl = function(vm, $mdDialog, RomService, rom){
  vm.rom = rom; 
